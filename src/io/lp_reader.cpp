...
enum class LpSection { None, Objective, Constraints, Bounds, General, Binary, End };
...
LpSection parseLpSection(const std::string& line) {
    ...
    if (lower == "subject to" || lower == "such that" || lower == "st" || lower == "s.t." ||
        lower == "subject to:") {
        return LpSection::Constraints;
    }
    if (lower == "bounds") {
        return LpSection::Bounds;
    }
...
/// pairs.
std::vector<std::pair<std::string, Real>> parseExpression(const std::vector<std::string>& tokens,
                                                          size_t& pos) {
    ...
    while (pos < tokens.size()) {
        ...
        Real coeff;
        auto [ptr, ec] = std::from_chars(tok.data(), tok.data() + tok.size(), coeff);
    ...
    for (size_t li = 0; li < lines.size(); ++li) {
        ...
        switch (section) {
            ...
            case LpSection::Constraints: {
                ...
                if (pos < tokens.size()) {
                    auto [ptr, ec] = std::from_chars(
                        tokens[pos].data(), tokens[pos].data() + tokens[pos].size(), rhs_val);
                    if (ec != std::errc{}) {
                        rhs_val = 0.0;
                    }
                }
            ...
            case LpSection::Bounds: {
                ...
                if (tokens.size() >= 2 && toLower(tokens.back()) == "free") {
                ...
                } else if (tokens.size() >= 5 && (tokens[1] == "<=" || tokens[1] == "<") &&
                           (tokens[3] == "<=" || tokens[3] == "<")) {
                    // lb <= x <= ub
                    Real lb = 0.0;
                    std::from_chars(tokens[0].data(), tokens[0].data() + tokens[0].size(), lb);
                    Index idx = getOrCreateCol(tokens[2]);
                    Real ub = 0.0;
                    std::from_chars(tokens[4].data(), tokens[4].data() + tokens[4].size(), ub);
                    ...
                    Index idx = getOrCreateCol(tokens[0]);
                    Real val = 0.0;
                    std::from_chars(tokens[2].data(), tokens[2].data() + tokens[2].size(), val);
