...
std::optional<HeuristicSolution> ProximitySearchHeuristic::run(
    [[maybe_unused]] const LpProblem& problem, [[maybe_unused]] DualSimplexSolver& lp,
    [[maybe_unused]] std::span<const Real> primals, [[maybe_unused]] Real incumbent) {
    // Proximity search requires an incumbent solution.
    return std::nullopt;
}

std::optional<HeuristicSolution> ProximitySearchHeuristic::run(
    const LpProblem& problem, DualSimplexSolver& lp, [[maybe_unused]] std::span<const Real> primals,
    Real incumbent, std::span<const Real> incumbent_values) {
