    ...
    for (Index i = 0; i < problem.num_rows && accepted < max_cuts_per_family_; ++i) {
        ...
        for (Int k : k_values) {
            ...
            for (Index p = 0; p < row.size(); ++p) {
                ...
                // rounding is a relaxation only while floor(t*a_j) <= t*a_j:
                // the rounded term must not exceed the original, or the
                // left-hand side is strengthened and flooring the right-hand
                // side is no longer justified. Adding a tolerance before the
                // floor rounds a coefficient just below an integer *up*, which
                // strengthens that term by up to the tolerance times the
                // column's upper bound and can cut off integer-feasible points.
                const Real rounded = std::floor(inv_k * a);
                if (rounded <= 0.0) {
                    continue;
                }
