...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--time-limit", type=float, default=7200)
...
def main() -> int:
    ...
    print(
        f"[mittelman-mip] Running {len(instances)} MIPLIB instances "
        f"(threads={args.threads}, time_limit={args.time_limit:g}s)"
    )
    ...
    if args.node_limit > 0:
        solver_args.extend(["--node-limit", str(args.node_limit)])
    ...
    with out_path.open("w", encoding="utf-8", newline="") as f:
        ...
        for inst in instances:
            ...
            for _ in range(args.repeats):
                cmd = [str(bin_path), str(inst), *solver_args, *args.solver_arg]
            ...
            med_w = median(works)
            med_n = median(nodes)
            med_li = median(lp_iters)
            w.writerow([name, f"{med_t:.6f}", f"{med_w:.6f}", f"{med_n:.6f}", f"{med_li:.6f}", status])
            print(f"  {name}: {med_t:.6f}s, {med_w:.6f} wu, {med_n:.6f} nodes, {status}")

    print(f"Wrote Mittelman MIP benchmark CSV: {out_path}")
    return 0
