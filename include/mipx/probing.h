...
public:
    ...
    ProbingStats probe(const LpProblem& problem,
    ...
    [[nodiscard]] const std::vector<std::pair<Index, Real>>& fixings() const {
        return fixings_;
    }
    ...
    /// Get dominated variable pairs. dominated[i] = dominator means
    /// variable i is dominated by variable dominator.
...
private:
    ...
    void learnImplications(Index fixed_var, bool fixed_val,
                           const LpProblem& problem,
                           DomainPropagator& dp,
                           ImplicationGraph& graph,
                           ProbingStats& stats);
    ...
    void learnVariableBounds(Index fixed_var, bool fixed_val,
                             const LpProblem& problem,
                             DomainPropagator& dp,
                             VariableBoundStore& vb_store,
    ...
    void detectDominated(const ImplicationGraph& graph,
                         ProbingStats& stats);
