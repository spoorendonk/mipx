...
7. Intra-iteration parallel simplex (SIP-style), gated by model-shape wins and numerical stability — **in progress** (dual-infeasibility scan, CHUZC candidate scan, and CHUZR leaving-row scan now have TBB-backed parallel paths behind runtime options; default remains off)
